	...
	<div class="flex-col w-9/12 lg:w-1/2">
		<div class="tabs ml-5">
			<a
				...
				class:tab-active={$page.url.pathname == '/query/role'}
				href="/query/role">Rol</a
			>
			<a
				class="tab tab-lifted"
				class:tab-active={$page.url.pathname == '/query/parameters'}
				href="/query/parameters">Parametros</a
			>
		</div>
					...
					<ul class="menu p-4 w-80 bg-base-100 text-base-content h-full">
						{#each historyEntries as history}
							{#if history.type === 'role' && $page.url.pathname == '/query/role'}
								<li>
									<button on:click|preventDefault={() => historySelectCallback(history)}
										>{history.data.role}</button
									>
								</li>
							{/if}
							{#if history.type === 'parameter' && $page.url.pathname == '/query/parameters'}
								<div class="max-h-96">
									<li>
										<button on:click|preventDefault={() => historySelectCallback(history)}
											>{`C-${history.data.connection_string}_P-${history.data.pages}`}</button
										>
									</li>
								</div>
							{/if}
