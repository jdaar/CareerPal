import type { GetJobInfoCallback, GetJobLinksCallback, TJobInfo, Platform } from '../lib/platform';
import { log } from '../lib/io';
import { getListItemsByXPath, getMultipleTextByXPath, getTextByXPath, sleep } from '../lib/helpers';
...
export const getJobLinks: GetJobLinksCallback = async (urlPP) => {
	...
	for (let i = 0; i < urlPP.parameters.pages; i++) {
		...
		const jobLinks: string[] = await urlPP.page.evaluate(() => {
			const links: string[] = [];
			document
				.querySelectorAll('a[href^="/ofertas-de-trabajo/oferta-de-trabajo-de-"]')
				.forEach((link) => {
					links.push((link as HTMLAnchorElement).href);
				});
			return links;
		});
		log('debug', 'getJobLinks', `Found ${jobLinks.length} job links for page ${i + 1}.`);
...
export const getJobInfo: GetJobInfoCallback = async (urlPP) => {
	...
	const jobInfo: TJobInfo = {
		...
		company: (await getTextByXPath(urlPP.page, '/html/body/main/div[1]/p')).split('-')[0].trim(),
		location: (await getTextByXPath(urlPP.page, '/html/body/main/div[1]/p')).split('-')[1].trim(),
		salary: (
		...
		experience: (
			await getListItemsByXPath(urlPP.page, '/html/body/main/div[2]/div/div[2]/div[3]/ul')
		).filter((tag) => tag.includes('experiencia'))[0],
		technologies: await getTags(
			await getTextByXPath(urlPP.page, '/html/body/main/div[2]/div/div[2]/div[3]/p[1]'),
...
const platform: Platform = {
	name: 'computrabajo',
	getUrl: (role: string) => `https://co.computrabajo.com/trabajo-de-${role}-en-medellin`,
	getJobLinks,
	getJobInfo
};
