...
export async function objectsToCsv(
	...
	log('info', 'objectsToCsv', `Writing ${objects.length} objects to ${filePath}...`);
	...
	try {
		...
		log(
			'error',
			'objectsToCsv',
			`Error writing ${objects.length} objects to ${filePath}: ${(error as Error).message}`
		);
		throw error;
	}
	log('info', 'objectsToCsv', `Done writing ${objects.length} objects to ${filePath}.`);
}
...
export function log(
	...
	const logPrefix = `${new Date().toISOString()} ${colors[level]}[${level.toUpperCase()}]${
		colors.reset
	} ${colors.debug}[${functionName}]${colors.reset}`;
